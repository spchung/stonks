...
class CandlestickFinder(object):
    ...
    def has_pattern(self,
        ...
        if self.is_data_prepared:
            ...
            t_df = pd.DataFrame(results, columns=['row', self.target]).set_index('row')
            t_df.index =pd.MultiIndex.from_tuples(t_df.index, names=['symbol', 'timestamp'])
            candles_df = candles_df.join(t_df, how='outer')
            return candles_df
        else:
